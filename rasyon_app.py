...
# --- 2. DİNAMİK HESAPLAMALAR VE KLİNİK SINIRLAR ---
...
ihtiyac_ndf_min = round(ihtiyac_km * 0.28, 1) # Rumen sağlığı için KM'nin en az %28'i NDF olmalı

# --- 3. VERİLECEK YEMLER VE KLİNİK ÇÖZÜCÜ ---
with col_yem:
...
if btn_coz:
    ...
    # KLİNİK KISIT 2: Kalsiyum / Fosfor Dengesi (Ca/P oranı 1.5 ile 2.0 arasında olmalı)
    ...
    # KLİNİK KISIT 3: Rumen Sağlığı (NDF Limiti)
    ...
    if pulp.LpStatus[prob.status] == "Optimal":
        optimal_mi = True
        st.toast("✅ Klinik Olarak Güvenli ve Optimal Rasyon Bulundu!")
        ...
        st.error("⚠️ HATA: Seçtiğiniz yemlerle metabolik dengeyi (Ca/P ve NDF) bozmayacak bir rasyon oluşturulamıyor. Yem çeşitliliğini veya Max limitlerini artırın.")
...
with col_denge:
    ...
    denge_verileri = {
        "Klinik Parametre": ["Kuru Madde (kg/gün)", "Ham Protein (g/gün)", "ME (Mcal/gün)", "Kalsiyum (g/gün)", "Fosfor (g/gün)", "NDF (kg/gün)"],
        ...
        "Sonuç": [
            ...
            durum_hesapla(rasyon_ndf, ihtiyac_ndf_min)
        ]
    }
